...
def save_fractal():
    global FRACTAL
    if FRACTAL != None and FRACTAL.img != None: #проверка на то, что изображение было создано
...
def main():
    ...
    frame_left.buttons(lambda: draw_fractal(entry_red, entry_green, entry_blue,
                                            entry_width, entry_height, entry_xmin,
                                            entry_xmax, entry_ymin, entry_ymax,
                                            entry_number, canvas, gallery), save_fractal)
    frame_right.clear_button(lambda: clear_gallery(gallery))
